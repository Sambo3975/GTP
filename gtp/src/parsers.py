...
class GTPTransformer(Transformer):
    ...
    def iadd(self, lhs: Variable, rhs: Any):
        return lhs.set(self.add(lhs, rhs))
    ...
    def ipow(self, lhs: Variable, rhs: Any):
        return lhs.set(self.pow(lhs, rhs))
    ...
    def le(self, lhs: Any, rhs: Any):
        return self._eval(lhs) <= self._eval(rhs)

    def ge(self, lhs: Any, rhs: Any):
        return self._eval(lhs) >= self._eval(rhs)
    ...
    def post_inc(self, v: Variable):
        val = self._eval(v)
        v.set(val + 1)
        return val

    def post_dec(self, v: Variable):
        val = self._eval(v)
        v.set(val - 1)
        return val
